...
- `useSearch` hook: owns all search state (query, results, isLoading, error)
- `SearchInput`: controlled input, emits string values via onChange
- `App`: reads hook state, renders correct child component
- `mockApi.searchItems`: async function with variable 150-500 ms delay
- No global state, no external state library

## File Map

| File | Status |
|---|---|
| `src/hooks/useSearch.ts` | **Incomplete** -- candidate implements |
| `src/components/SearchInput.tsx` | Complete |
| `src/components/ItemCard.tsx` | Complete |
| `src/components/ItemList.tsx` | Complete |
| `src/components/LoadingState.tsx` | Complete |
| `src/components/EmptyState.tsx` | Complete |
| `src/services/mockApi.ts` | Complete (mock) |
| `src/types/index.ts` | Complete |
| `src/App.tsx` | One TODO -- conditional rendering |

## Key Notes

- `searchItems` has intentionally variable latency to expose race conditions
- The hook should debounce by 300 ms before issuing a call
- Stale responses must be discarded; the last-issued request wins
- All cleanup (timers, cancelled flags) must run on unmount
